...
class ClientStore(object):
    ...
    async def auth_client_registration(self, request: sanic.request.Request) -> bool:
        ...
        if self._provider.client_registration_key is None:
            ...
            isinstance(self._provider.client_registration_key, str) and self._provider.client_registration_key != token
        ):
            logger.warning("Client attempted registration without incorrect Bearer token {0}".format(masked(token)))
            return False
        elif (
            inspect.iscoroutinefunction(self._provider.client_registration_key)
    ...
    async def validate_client(
        self, id_: str, name: str, secret: str, type_, callback_urls: Tuple[str, ...]
    ) -> Tuple[bool, str]:
