...
async def secret(request: sanic.request.Request) -> sanic.response.BaseHTTPResponse:
    ...
    headers = {"Authorization": "Bearer " + user_session["user"]["access_token"], "Content-Type": "application/json"}
...
async def index(request: sanic.request.Request) -> sanic.response.BaseHTTPResponse:
    return sanic.response.html(
        '<html><body>Homepage, secret page: <a href="{0}">/secret</a></body></html>'.format(app.url_for("secret"))
    )
