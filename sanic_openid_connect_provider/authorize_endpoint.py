...
from sanic_openid_connect_provider.exceptions import AuthorizeError, ClientIdError, RedirectUriError
from sanic_openid_connect_provider.utils import get_provider, get_scheme, redirect, strip_prompt_login
...
async def create_authorize_response_params(
    request: sanic.request.Request, params: Dict[str, Any], user: Dict[str, Any]
) -> Tuple[dict, dict]:
...
def create_authorize_response_uri(
    redirect_uri: str, query_params: Dict[str, Any], query_fragment: Dict[str, Any]
) -> str:
...
async def validate_authorize_params(request: sanic.request.Request, provider: "Provider") -> Dict[str, Any]:
    ...
    if grant_type is None:
        logger.warning("grant_type missing for client {0}".format(client.name))
    ...
    try:
        ...
        if await provider.users.is_authenticated(request):
            ...
            allow_skipping_consent = (
                params["client"].type in ("public", "pairwise") or params["response_type"] in implicit_flow_resp_types
            )
            ...
            if params["client"].reuse_consent:
                ...
                if user["consent"] and allow_skipping_consent and "consent" not in params["prompt"]:
                    ...
                    if params["response_mode"] == "form_post":
                        ...
                        return redirect(
                            create_authorize_response_uri(params["redirect_uri"], query_params, query_fragment)
                        )
            ...
            hidden_inputs = await request.app.extensions["jinja2"].render_string_async(
                provider.hidden_inputs_html, request, params=hidden_params
            )
