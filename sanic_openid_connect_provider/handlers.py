...
from sanic_openid_connect_provider.exceptions import TokenError
...
from sanic_openid_connect_provider.utils import get_provider, get_scheme
from sanic_openid_connect_provider.validation import validate_userinfo_params
...
async def well_known_oauth_config_handler(request: sanic.request.Request) -> sanic.response.BaseHTTPResponse:
    ...
    response = {
        ...
        "authorization_endpoint": request.app.url_for(
            "authorize_handler", _scheme=scheme, _external=True, _server=request.host
        ),
        ...
        "registration_endpoint": request.app.url_for(
            "client_register_handler", _scheme=scheme, _external=True, _server=request.host
        ),
        ...
        "introspection_endpoint": request.app.url_for(
            "introspection_handler", _scheme=scheme, _external=True, _server=request.host
        ),
...
async def well_known_openid_config_handler(request: sanic.request.Request) -> sanic.response.BaseHTTPResponse:
    ...
    response = {
        ...
        "token_endpoint": request.app.url_for("token_handler", _scheme=scheme, _external=True, _server=request.host),
        "userinfo_endpoint": request.app.url_for(
            "userinfo_handler", _scheme=scheme, _external=True, _server=request.host
        ),
        "jwks_uri": request.app.url_for("jwk_handler", _scheme=scheme, _external=True, _server=request.host),
        "registration_endpoint": request.app.url_for(
            "client_register_handler", _scheme=scheme, _external=True, _server=request.host
...
async def well_known_finger_handler(request: sanic.request.Request) -> sanic.response.BaseHTTPResponse:
    ...
    try:
        ...
        return sanic.response.json(
            resp, content_type="application/jrd+json", headers={"Access-Control-Allow-Origin": "*"}
        )
    ...
    if "client_id" in request.args:
        ...
        try:
            ...
            return sanic.response.text(
                body="", status=403, headers={"WWW-Authenticate": 'Bearer error="invalid_token"'}
            )
        ...
        if sector_identifier_uri:
            try:
                ...
                result = {
                    ...
                    "error_description": "Failed to validate sector identifier uri, {0}".format(err),
                }
                return sanic.response.json(result, status=400)
        ...
        if success:
            ...
            result = {
                ...
                "registration_client_uri": request.app.url_for(
                    "client_register_handler", _scheme=scheme, _external=True, _server=request.host, client_id=client_id
                ),
                "registration_access_token": client.access_token,
            ...
            result = {"error": "invalid_client_metadata", "error_description": data}
            status = 500
